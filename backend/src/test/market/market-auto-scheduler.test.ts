...
test("scheduler groups assets by market and does at most one Yahoo batch call per market", () => {
  ...
  assert.ok(result.runs.every((run: { open_status: string }) => run.open_status === "confirmed_open"));
});
...
test("late server start after close marks missed_open_window instead of holiday_suspected", () => {
  ...
  assert.ok(result.logs.some((log: { message: string; phase: string }) => log.phase === "open" && log.message === "missed_open_window"));
});
...
test("close confirmation refreshes snapshots before one unique post-close finalization", () => {
  ...
  assert.ok(result.tasks.every((task: { market_key: string }) => task.market_key === "euronextParis"));
  assert.ok(result.tasks.every((task: { trading_date: string }) => task.trading_date === "2026-05-06"));
  assert.ok(result.tasks.every((task: { phase: string }) => task.phase === "close"));
  assert.ok(result.tasks.every((task: { task_key: string }) => task.task_key.startsWith("EURONEXTPARIS:2026-05-06:CLOSE:")));

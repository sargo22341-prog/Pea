...
test("queue construction persiste, dedupe les taches actives et reprend une tache interrompue", () => {
  ...
  assert.ok(result.statuses.every((row: { status: string }) => row.status === "success"));
  assert.equal(result.statuses.find((row: { attempts: number; task_key: string }) => row.task_key === "RESTART:AAA.PA:SNAPSHOT")?.attempts, 2);

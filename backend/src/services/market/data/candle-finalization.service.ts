...
/**
 * `CandleFinalizationService` (anciennement `PostCloseFinalizationService`) : finalise la
 * dernière candle 1d après fermeture de marché en attribuant un prix de clôture définitif
 * (`close source = snapshot_close` ou `yahoo_daily_fallback_close`), met à jour
 * `market_data_finalizations`, et résout les sessions intraday closes.
 *
 * Une opération unique par (asset, trading_date) — protégée par le lock `candles:${symbol}`
 * dans le service appelant.
 */
...
export const candleFinalizationService = new CandleFinalizationService();

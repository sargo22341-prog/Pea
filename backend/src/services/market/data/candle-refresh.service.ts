...
import { candleFinalizationService } from "./candle-finalization.service.js";
import { marketDataGateway } from "./market-data-gateway.service.js";

/**
 * `CandleRefreshService` : refresh complet (1d/1w/1m/all) des candles pour un asset.
 *
 * - Construit la fenêtre temporelle pour chaque range, fetch Yahoo, sanitize, upsert.
 * - Délègue la finalisation post-close à `candleFinalizationService`.
 * - Sérialise par symbole via `symbolLockService` (lock `candles:${symbol}`) pour éviter les
 *   races avec `LiveChartService.refreshForAsset` ou `rebuildStoredRangesFromFinalData`.
 */
export class CandleRefreshService {
  refreshCandlesForAsset(asset: AssetRow, ranges: StoredChartRange[] = storedConstructionRanges) {
    ...
    for (const range of ranges) {
      ...
      if (range === "1d" && validatedPoints.length === 0) {
        ...
        validatedPoints = latestYahooTradingDay
          ? await candleFinalizationService.fetchClosedIntradaySession({ asset, tradingDay: latestYahooTradingDay, quote, persist: false })
          : [];
  ...
  private async finalizePostCloseForAssetLocked(asset: AssetRow, now: Date) {
    ...
    return candleFinalizationService.finalizeClosedOneDayCandles({
      asset,
      session,
      quote,
      yahooTradingDay,
...
export const candleRefreshService = new CandleRefreshService();
